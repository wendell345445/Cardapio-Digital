...
- `GET /api/print/pending` → `{ orders: [{ id, orderId, orderNumber, data }] }` — polling do operador. **v2.10: o campo `data` é JSON estruturado** (`ReceiptData` em [admin/print.service.ts](api/src/modules/admin/print.service.ts)), substituiu o antigo `receipt` string pré-formatado. A montagem do cupom (largura/fonte/quebra de linha) passou pro lado do Menuziprinter (`buildOrderHtml` já existe no app — basta o poller trocar `buildReceiptHtml(order.receipt)` por `buildOrderHtml(order.data)`). Shape de `data`: `{ orderNumber, createdAt(ISO), type, typeLabel, customerName, customerPhone, tableNumber, customerAddress, paymentMethod, paymentLabel, items:[{ name, quantity, totalPrice, options:[{name,price}], notes }], subtotal, discount, deliveryFee, total, notes }`.
